...
class App {
    ...
    renderMiniSubstitutionTable(teacherData, date, showDeleteAll = false) {
        ...
        const tableRows = timeSlots.map(slot => {
            ...
            const exceptionalSubstituteName = scheduleItem?.exceptionalSubstitute;
            ...
            const hasSub = !!(substituteName || exceptionalSubstituteName);
            ...
            // Construir celda de sustitutos (puede haber uno o dos)
            let substituteCellContent = '';
            if (substituteName) {
                substituteCellContent += `<div style="font-size: 0.75rem; color: #166534; font-weight: 500;">${substituteName}</div>`;
            }
            if (exceptionalSubstituteName) {
                substituteCellContent += `<div style="font-size: 0.7rem; color: #7c3aed; font-weight: 500; margin-top: 2px;">${exceptionalSubstituteName} (CE)</div>`;
            }
            ...
            const substituteCell = hasSub 
                ? `<div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 0.5rem;">
                     <div style="cursor: pointer; flex: 1;"
                        ...
                        title="Click para cambiar sustituto">
                        ${substituteCellContent}
                     </div>
    ...
    editSubstitutionSlot(teacherId, date, startTime, endTime, subId) {
        ...
        setTimeout(() => {
            if (sub.schedule && sub.schedule[0]) {
                ...
                // Seleccionar el sustituto excepcional si existe
                if (slot.exceptionalSubstitute) {
                    const excSubTeacher = store.state.teachers.find(t => t.name === slot.exceptionalSubstitute);
                    if (excSubTeacher) {
                        document.getElementById('exceptional-substitute').value = excSubTeacher.id;
                    }
                }
            }
        }, 100);
            ...
            <div style="max-width: 600px; margin: 0 auto;">
                ...
                <div class="card" style="margin-bottom: 1.5rem;">
                    ...
                    <form id="new-sub-form" onsubmit="app.handleNewSubSubmit(event)">
                        ...
                        <!-- Sustituto Casos Excepcionales -->
                        <div style="margin-bottom: 1.25rem;">
                            <label style="display: block; margin-bottom: 0.5rem; font-weight: 500; color: #374151;">Sustituto Casos Excepcionales</label>
                            <select id="exceptional-substitute"
                                    style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 0.875rem;">
                                <option value="">Seleccionar sustituto excepcional...</option>
                                ${teacherOptions}
                            </select>
                            <div style="margin-top: 0.25rem; font-size: 0.75rem; color: #6b7280; font-style: italic;">
                                Todos los profesores del centro disponibles para casos excepcionales
                            </div>
                        </div>
    ...
    async handleNewSubSubmit(e) {
        ...
        const exceptionalSubstituteId = document.getElementById('exceptional-substitute').value;
        const courseSubject = document.getElementById('course-subject').value;
        const reason = document.getElementById('reason').value;
        
        // Validations
        if (!date || !startTime || !endTime || !absentTeacherId || !courseSubject) {
            alert('Por favor, complete todos los campos obligatorios');
            return;
        }
        
        // Validar que al menos un sustituto esté seleccionado
        if (!substituteTeacherId && !exceptionalSubstituteId) {
            alert('Debe seleccionar al menos un profesor sustituto (Profesor Sustituto o Sustituto Casos Excepcionales)');
            return;
        }
        
        // Validate substitute is not the absent teacher
        if (substituteTeacherId && String(substituteTeacherId) === String(absentTeacherId)) {
            alert('El profesor sustituto no puede ser el mismo que el profesor ausente');
            return;
        }
        
        // Validate exceptional substitute is not the absent teacher
        if (exceptionalSubstituteId && String(exceptionalSubstituteId) === String(absentTeacherId)) {
            alert('El sustituto para casos excepcionales no puede ser el mismo que el profesor ausente');
            return;
        }
        ...
        if (substituteTeacherId && assignedTeachers.includes(String(substituteTeacherId))) {
        ...
        // Validate exceptional substitute is not already assigned at this time
        if (exceptionalSubstituteId && assignedTeachers.includes(String(exceptionalSubstituteId))) {
            const subTeacher = store.state.teachers.find(t => String(t.id) === String(exceptionalSubstituteId));
            alert(`${subTeacher?.name || 'Este profesor'} ya está asignado a otra sustitución en este horario`);
            return;
        }
        ...
        if (substituteTeacherId && absentTeachersForDate.includes(String(substituteTeacherId))) {
            const subTeacher = store.state.teachers.find(t => String(t.id) === String(substituteTeacherId));
            alert(`${subTeacher?.name || 'Este profesor'} está marcado como ausente en esta fecha y no puede sustituir a otros`);
            return;
        }
        
        // Validate exceptional substitute is not marked as absent on this date
        if (exceptionalSubstituteId && absentTeachersForDate.includes(String(exceptionalSubstituteId))) {
            const subTeacher = store.state.teachers.find(t => String(t.id) === String(exceptionalSubstituteId));
            alert(`${subTeacher?.name || 'Este profesor'} está marcado como ausente en esta fecha y no puede sustituir a otros`);
            return;
        }
        ...
        const substituteTeacher = substituteTeacherId ? store.state.teachers.find(t => String(t.id) === String(substituteTeacherId)) : null;
        const exceptionalSubstitute = exceptionalSubstituteId ? store.state.teachers.find(t => String(t.id) === String(exceptionalSubstituteId)) : null;
        
        // Construir lista de sustitutos
        let substitutesList = [];
        if (substituteTeacher) {
            substitutesList.push(substituteTeacher.name);
        }
        if (exceptionalSubstitute) {
            substitutesList.push(exceptionalSubstitute.name + ' (CE)');
        }
        ...
        const substitution = {
            ...
            schedule: [{
                ...
                substitute: substituteTeacher ? substituteTeacher.name : null,
                exceptionalSubstitute: exceptionalSubstitute ? exceptionalSubstitute.name : null,
                allSubstitutes: substitutesList,
    ...
    renderNewFormWithPreFilledData(subId) {
        ...
        setTimeout(() => {
            if (sub.schedule && sub.schedule[0]) {
                ...
                // Find and set exceptional substitute teacher
                if (slot.exceptionalSubstitute) {
                    const excSubTeacher = store.state.teachers.find(t => t.name === slot.exceptionalSubstitute);
                    if (excSubTeacher) {
                        document.getElementById('exceptional-substitute').value = excSubTeacher.id;
    ...
    generatePrintTableHTML(teacherData, date) {
        ...
        const tableRows = timeSlots.map(slot => {
            ...
            // Construir celda de sustitutos para impresión
            let substituteCellContent = '';
            if (substituteName) {
                substituteCellContent += `<div class="print-substitute-name">${substituteName}</div>`;
            }
            if (exceptionalSubstituteName) {
                substituteCellContent += `<div class="print-exceptional-substitute">${exceptionalSubstituteName} (CE)</div>`;
            }
